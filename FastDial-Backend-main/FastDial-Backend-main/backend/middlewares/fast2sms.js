...
async function sendFast2OTP(number) {
  if (!smsId || !smsPass || !smsEmail) {
    console.log(`[DUMMY OTP] Sending dummy OTP 1234 to ${number}`);
    const dummyVid = "dummy_vid_" + Date.now();
    const sqlStatement = "INSERT INTO otp (mobile, vid) VALUES(?, ?)";
    await db(sqlStatement, [number, dummyVid]);
    return dummyVid;
  }
...
async function VerifyFast2OTP(vid, code) {
  if (vid && vid.toString().startsWith("dummy_vid_")) {
    return code.toString() === "1234";
  }
  const token = await getToken();
  const options = {
    method: "GET",
