...
const AutomaticLocationModal = ({ isOpen, onClose, onOpenManualLocationModal, formData, setFormData }) => {
    ...
    const toggleMapScreen = async () => {
        ...
        try {
            ...
            await getLocationList(latitude, longitude);
        } catch (error) {
            console.error("Location fetch failed:", error);
            alert("Could not fetch location automatically. Please enter it manually or check your Google Maps API Key.");
    ...
    async function getLocationList(latitude, longitude) {
        try {
            ...
            if (res.status !== "OK" || !res.results || res.results.length === 0) {
                throw new Error(res.error_message || "Failed to fetch geocoding data");
            }
